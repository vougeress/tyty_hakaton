...
function deriveGap(trip: TripDetails, events: CalendarEvent[]): CalendarGap[] {
  ...
  for (let index = 0; index < sorted.length - 1; index += 1) {
    ...
    const sameDay = zonedDateParts(previous.endsAt, trip.timezone).date === zonedDateParts(next.startsAt, trip.timezone).date;
    const gapMinutes = (next.startsAt.getTime() - previous.endsAt.getTime()) / 60_000;
    if (!sameDay || gapMinutes < 240) continue;
    ...
    return [{
      id: "demo-gap",
      tripId: trip.id,
      startsAt: new Date(previous.endsAt.getTime() + departureBufferMinutes * 60_000).toISOString(),
